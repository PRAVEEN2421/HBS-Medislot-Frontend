...
const Navbar = () => {
  ...
  return (
    <div className='sticky top-0 z-50 w-full mb-8'>
      {/* ---- Main Header Row ---- */}
      <div className='flex items-center justify-between text-sm py-4 border-b border-gray-100 bg-white/80 backdrop-blur-md transition-all duration-300 px-2 sm:px-6'>
        ...
        <div className='flex items-center gap-4'>
              ...
              ? <div className='flex items-center gap-3 cursor-pointer group relative'>
                ...
                <div className='absolute top-full right-0 pt-4 text-base font-medium text-gray-600 z-50 hidden group-hover:block'>
                  <div className='min-w-56 bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-gray-100 flex flex-col p-3 overflow-hidden animate-[fadeIn_0.2s_ease-out]'>
                    ...
                    <div onClick={() => navigate('/my-profile')} className='flex items-center gap-3 px-4 py-2.5 hover:bg-blue-50/80 hover:text-primary rounded-xl cursor-pointer transition-colors text-sm'>My Profile</div>
                    <div onClick={() => navigate('/my-appointments')} className='flex items-center gap-3 px-4 py-2.5 hover:bg-blue-50/80 hover:text-primary rounded-xl cursor-pointer transition-colors text-sm'>Appointments</div>
                    <div onClick={logout} className='flex items-center gap-3 px-4 py-2.5 hover:bg-red-50 hover:text-red-500 rounded-xl cursor-pointer transition-colors text-sm text-red-400'>Logout</div>
                  </div>
                </div>
              </div>
          ...
          <div onClick={() => setShowMenu(true)} className='md:hidden p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer'>
            <img className='w-6 opacity-70' src={assets.menu_icon} alt="Menu" />
          </div>
        </div>
      </div>

      {/* ---- Mobile Drawer (Fixed - Fully Independent) ---- */}
      <div
        className={`md:hidden fixed inset-y-0 right-0 z-[1001] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${showMenu ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className='h-full w-[280px] bg-white shadow-[-10px_0_30px_rgba(0,0,0,0.1)] flex flex-col relative'>
          {/* Menu Header */}
          <div className='flex items-center justify-between px-6 py-6 border-b border-gray-50'>
            <img src={assets.logo} className='w-36' alt="MediSlot" />
            <button
              onClick={() => setShowMenu(false)}
              className='w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center hover:bg-primary/10 hover:rotate-90 transition-all duration-300'
            >
              ...
              {[
                ...
                <NavLink
                  ...
                  className={({ isActive }) => `
                    ...
                    ${isActive ? 'bg-primary text-white shadow-lg' : 'hover:bg-gray-50 text-gray-600 hover:text-primary'}
                  `}
                >
          ...
          <div className='p-6 border-t border-gray-50 bg-gray-50/30'>
            {!token ? (
              <button
                ...
                className='w-full bg-primary text-white py-4 rounded-xl font-bold shadow-md'
              >
              ...
              <div className='flex flex-col gap-4'>
                <div className='flex items-center gap-3 px-2'>
                  <img className='w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm' src={userData.image} alt="" />
                  <div className='flex flex-col'>
                    <p className='text-sm font-bold text-gray-800 truncate max-w-[150px]'>{userData.name}</p>
                    <p className='text-[10px] text-primary font-medium uppercase tracking-tighter'>Verified Patient</p>
                  </div>
                </div>
                <button
                  onClick={logout}
                  className='w-full flex items-center justify-center gap-2 bg-white text-red-500 py-3 rounded-xl font-bold shadow-sm hover:bg-red-50 transition-colors'
                >
      ...
      <div
        onClick={() => setShowMenu(false)}
        className={`md:hidden fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-500 ${showMenu ? 'opacity-100 z-[1000]' : 'opacity-0 pointer-events-none z-[-1]'}`}
      ></div>
    </div>
  )
}
